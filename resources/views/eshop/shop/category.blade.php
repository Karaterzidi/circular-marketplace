            ...
            <div class="row">
              ...
              <div class="cats-sidebar col-lg-3 order-2 order-lg-1">
                ...
                <div @class([
                  ...
                  'px-4',
                  'mb-3',
                  'text-white' => $cat->id == $category->id,
                  'bg-dark' => $cat->id == $category->id,
                  'bg-light' => $cat->id != $category->id
                  ])><strong class="small text-uppercase font-weight-bold">{{$cat->title}}</strong></div>
                <ul class="list-unstyled small text-muted pl-lg-4 font-weight-normal">
                  @foreach($cat->subcategories as $subcat)
                  <li class="mb-2"><a class="reset-anchor" href="/shop/categories/{{$cat->slug}}/{{$subcat->slug}}">{{$subcat->title}}</a></li>
                  @endforeach
                </ul>
                @endforeach
              </div>
              ...
              <div class="col-lg-9 order-1 order-lg-2 mb-5 mb-lg-0">
                <div class="row mb-3 align-items-center">
                  <div class="col-lg-6 mb-2 mb-lg-0">
                    <h5 class="font-weight-bold mb-0">{{$category->title}} / {{$subcategory->title}}</h5>
                  </div>
                  <div class="col-lg-6">
                    <ul class="list-inline d-flex align-items-center justify-content-lg-end mb-0">
                      <li class="list-inline-item">
                        <select onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);" class="selectpicker ml-auto" name="sorting" data-width="200" data-style="bs-select-form-control" data-title="Sorting">
                          <option selected disabled>Sort By:</option>
                          <option value="/shop/categories/{{$category->slug}}/{{$subcategory->slug}}?sortBy=low">Price: Low to High</option>
                          <option value="/shop/categories/{{$category->slug}}/{{$subcategory->slug}}?sortBy=high">Price: High to Low</option>
                        </select>
                      </li>
                    </ul>
                  </div>
